...
static NSString * const kchatterbox_alphaJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)_chatterbox_alphaInjection
{
	...
	[html appendString:kchatterbox_alphaJavascript];
...
{
	// attempt to make well-formed HTML and inject in our chatterbox_alpha bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self _chatterbox_alphaInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to chatterbox_alpha support.",url);
				}
			}
